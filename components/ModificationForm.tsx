      ...
      {isOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-8 rounded-2xl w-full max-w-md shadow-2xl border border-slate-200">
            ...
            <p className="text-slate-500 mb-6 text-sm">Ayudanos a mantener la oficina judicial actualizada.</p>
            ...
            {status === 'success' ? (
              ...
              <form action={handleFormAction} className="flex flex-col gap-4">
                ...
                {/* NUEVO SELECT: TIPO DE SOLICITUD */}
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-slate-600 uppercase">Tipo de reporte</label>
                  <select 
                    name="tipo" 
                    required 
                    className="border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-slate-800 bg-white"
                  >
                    <option value="modificacion">Modificar oficina existente</option>
                    <option value="nueva">Sugerir nueva oficina</option>
                  </select>
                </div>
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-semibold text-slate-600 uppercase">Nueva Ubicación / Datos</label>
                  <textarea name="ubicacion" placeholder="Piso, pasillo o descripción detallada..." required className="border border-slate-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none h-28 resize-none text-slate-800" />
                </div>
